...
<div class="container">
  <div class="act-netting">
    ...
    <p class="indent-size">
      ...
      договору-оферте №{{ $offer->id }} от «{{ date('d', strtotime($offer->accept_date)) }}» {{ __('dates.dec')[1 * date('m', strtotime($offer->accept_date)) - 1] }} {{ date('Y', strtotime($offer->accept_date)) }} г. по состоянию на «{{ date('d') }}» {{ __('dates.dec')[1 * date('m', strtotime("$year-$month-01 + 1 month")) - 1] }} {{ $year }} г.
    </p>
    ...
    <p class="indent-size">
      Стороны согласились произвести взаимозачёт по вышеупомянутому договору в сумме {{ abs($credit - $debit) }} ({{ sum_to_russian_string(abs($credit - $debit)) }}) рублей.
    </p>

    <p class="indent-size">
      Незачтённая сумма в размере {{ abs($credit - $debit) }} ({{ sum_to_russian_string(abs($credit - $debit)) }}) рублей
