<template>
  <div class='user-info'>
    ...
    <div class='errors' v-if='errors.phone'>{{ errors.phone }} <a v-if='errors.phone_link' href='javascript:void(0);'  @click='login'>{{ __('header.log_in') }}</a></div>
    <input v-bind:disabled='!!user'  placeholder='E-mail' v-model='email' />
    <div class='errors' v-if='errors.email'>{{ errors.email }} <a v-if='errors.email_link' href='javascript:void(0);' @click='login'>{{ __('header.log_in') }}</a></div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      login() {
        this.$store.commit('Header/setPhone', this.phone);
        this.$store.commit('Header/Appearance/show');
      },
    }
  }
</script>