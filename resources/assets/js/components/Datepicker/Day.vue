  ...
  export default {
    ...
    methods: {
      click () {
        ...
        if (this.from.toISODateString() === this.date.toISODateString() && this.type === 'till') return;
        this.$emit('input', {
          type: this.type,
          value: this.date
        });
      ...
      enter () {
        this.$emit('msenter', this.date);
      }
    },
    computed: {
      inactive () {
        return !this.loose && this.date.toISODateString() < new Date().toISODateString();
      },
      picked () {
