<template>
  <div class='countryinput' @click.stop.prevent='hide'>
    <div class='country-selected' @click='toggle'>
    ...
    <input name='phone' :disabled='disabled' placeholder='__("common.phone_number")' type='tel' maxlength='20' @input.prevent='' v-model='phone' />
  </div>
</template>

<script>
  import { mapState, mapActions, mapMutations, mapGetters } from 'vuex';
  import countries from './../../components/RoompHeader/countries.js';
  import Bus from '../../Bus';
  ...
  export default {
    ...
    computed: {
      country () {
        return this.countries.where('code', this.code);
      },
      codePhone () {
        if (this.country) return this.country.phone + ' ';
        return '';
      },
      phoneFormatted () {
        const format = str => {
          ...
          switch (true) {
            case (len <= 3): return str;
            case (len <= 6): return str.substr(0,3) + '-' + str.substr(3,3);
            case (len <= 8): return str.substr(0,3) + '-' + str.substr(3,3) + '-' + str.substr(6,2);
            case (len <= 10): return str.substr(0,3) + '-' + str.substr(3,3) + '-' + str.substr(6,2) + '-' + str.substr(8,2);
          }
        };
        
        return '+' + this.codePhone + format(this.phoneValue);
      },
      phone: {
        get() {
        ...
        set(val) {
          this.caretPos = this.input.value.length - Math.max(this.input.selectionStart, this.input.selectionEnd);
          
          let value = val.replace(/[\D]/ig,'');
          ...
          if (!country || country.length === 0 || value.length === 0) {
            this.code = null;
            country = null;
          } else {
            value = value.replace(new RegExp(country.phone), '');
          }
          
          if (value.length > 10) value = value.substr(0,10);
          
          if (value.length === 0 && country) return;
    ...
    methods: {
      ...
      forceUpdateValue() {
        ...
        this.input.setSelectionRange(this.phone.length - this.caretPos, this.phone.length - this.caretPos);
    ...
    mounted () {
      ...
      if (this.phoneInit) {
        ...
        this.phoneValue = init.substr(codeLen, 10);
