<template>
  <div class="date_filter">
    <h5>{{__('extranet.date_filter.name')}}</h5>
    <select @input="e => switchType(e.target.value)" v-if="$router.history.current.path !== '/reconciliation'">
      <option :value="FilterType.Creation">{{__('extranet.date_filter.created_at')}}</option>
      <option :value="FilterType.Arrival">{{__('extranet.date_filter.arrival')}}</option>
      <option :value="FilterType.Departure">{{__('extranet.date_filter.departure')}}</option>
    </select>
